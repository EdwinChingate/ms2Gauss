...
The goal is not to capture the strongest peaks — but the most reproducible structural evidence.
...
Developer & Maintainer: Edwin Chingate (Colombia)

ms2Topo originated from my PhD research on microbial transformation of pharmaceutical compounds using LC–HRMS (DDA). While analyzing years of MS/MS data, I found that common workflows often treat redundant DDA spectra as “noise” and rely heavily on MS1-first feature definitions. I built ms2Topo to extract *structural certainty* directly from fragmentation evidence by treating redundant MS2 spectra as statistical replication and declaring features only when fragmentation patterns form stable, coherent communities in a similarity network.
